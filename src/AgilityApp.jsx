  ...
  return (
    <div className="min-h-screen bg-[#fffcfa]">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-md mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm opacity-60">Agility planning</span>
          </div>
          <div className="flex items-center gap-3">
            <button className="w-6 h-6">
              <img src={profileSvg} alt="Profile" className="w-full h-full" />
            </button>
            <button className="w-6 h-6 opacity-60">
      ...
      <main className="max-w-md mx-auto px-4 py-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {activities.map((activity, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <div className="w-20 text-xs text-gray-600 text-right pr-3">
                  {activity.name}
                </div>
                <div className="flex-1 flex justify-between px-1">
                  {activity.days.map((completed, dayIdx) => (
                    <div key={dayIdx} className="w-6 flex justify-center">
                      <div className={`w-3 h-3 rounded-full ${
                        completed ? 'bg-[#8590C8]' : 'bg-gray-200'
                      }`}></div>
                    </div>
                  ))}
                </div>
        ...
        {/* In Progress Card */}
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <h3 className="text-sm font-bold text-gray-900 mb-1">In progress</h3>
